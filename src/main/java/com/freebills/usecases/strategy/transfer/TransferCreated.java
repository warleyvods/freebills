...
@Component(value = "TRANSFER_CREATED")
public class TransferCreated implements BalanceUpdateStrategy {
    ...
    private boolean isTransferIn(Event event) {
        return event.getEventType() == TRANSFER_CREATED && event.getTransferData().getTransferType() == IN;
    }

    private boolean isTransferOut(Event event) {
        return event.getEventType() == TRANSFER_CREATED && event.getTransferData().getTransferType() == OUT;
    }
}
